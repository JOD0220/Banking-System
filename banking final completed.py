...
def Menu():
    ...
    print("*"*140)
def SortAcc(F):          #function to arrange records as per ascending order of account number
    try:
...
def SortName(F):         #function to arrange records as per ascending order of customer number
    try:
        with open(F,'rb+') as fil:
            rec=pickle.load(fil)
        ...
        print(F, "File has no records")
def SortBal(F):          #function to arrange records as per ascending order of customer balance
    try:
        with open(F,'rb+') as fil:
            rec=pickle.load(fil)
...
def SearchAcc(F): #Function to Search for the Record from the File with respect to the account number
    try:
        with open(F,'rb') as fil:
            Rec=pickle.load(fil)
            ...
            print(F, "File Doesn't exist")
def SearchName(F): #Function to Search for the Record from the File with respect to the customer name
    try:
        with open(F,'rb') as fil:
            Rec=pickle.load(fil)
...
def Debit(F): #Function to Withdraw the amount by assuring the min balance of Rs 5000
    try:
        with open(F,'rb+') as fil:
            Rec=pickle.load(fil)
